...
export default function HistoryAbsenPegawai() {
  ...
  const renderStatusBadge = (status?: string) => {
    ...
    switch (status) {
      ...
      case "cuti":
        return <span className={`${base} bg-indigo-100 text-indigo-700`}>Cuti</span>;
      case "libur":
        return <span className={`${base} bg-gray-200 text-gray-700`}>Libur</span>;
      case "alpha":
        return <span className={`${base} bg-red-100 text-red-700`}>Alpha</span>;
      case "sudah_masuk":
        return <span className={`${base} bg-purple-100 text-purple-700`}>Sudah Masuk</span>;
      case "sudah_pulang":
        return <span className={`${base} bg-green-100 text-green-700`}>Sudah Pulang</span>;
      default:
        return (
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-5 pb-10 rounded-b-[32px] shadow-lg pt-8">
          <div className="flex items-center">
            ...
            <div className="flex-1 text-center">
              <h1 className="text-lg font-semibold">History Absen {pegawai?.name || ""} </h1>
            </div>
                ...
                {filteredAbsen.map((a, i) => (
                  <tr key={a.id} className="border-b">
                    ...
                    <td className="p-3 text-center space-y-1">
                      {renderStatusBadge(a.status)} dan      {/* status DB */}
                      {renderProgressBadge(a.status_absen)} {/* progress */}
                    </td>
                  </tr>
                ))}
